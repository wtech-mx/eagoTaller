<?php
	...
	$errors[]= "<p>Lo sentimos, pero el archivo es demasiado grande. Selecciona logo de menos de 10MB</p>";
		...
		$errors[]= "<p>ID del factura 1 está vacío.</p>";
	} else{
		/* Fin Validacion*/
		if ($imageFileZise>0){
		move_uploaded_file($_FILES["imagefile3"]["tmp_name"], $target_file);
		$imagen=basename($_FILES["imagefile3"]["name"]);
		$img_update="foto3='view/resources/images/documentos/$image_name' ";
...
?>
			<a href="" class="img-responsive" alt="Imagen del factura 1" data-toggle="modal" data-target="#myModal3" style="cursor:pointer;padding: 10px;background: #2ECC71;overflow-y: hidden;overflow-x: hidden;">
...
<?php
			...
			?>
				<img class="img-responsive" src="view/resources/images/documentos/<?php echo $image_name;?>" alt="Imagen del factura origen" data-toggle="modal" data-target="#myModal3" style='cursor:pointer'>
<?php
	}
?>
