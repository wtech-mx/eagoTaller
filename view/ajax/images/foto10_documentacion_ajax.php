<?php
	...
	$image_name = time()."_".basename($_FILES["imagefile10"]["name"]);
	$target_file = $target_dir .$image_name ;
	$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
	$imageFileZise=$_FILES["imagefile10"]["size"];
	...
	$errors[]= "<p>Lo sentimos, pero el archivo es demasiado grande. Selecciona logo de menos de 10MB</p>";
		...
		$errors[]= "<p>ID del tenecia está vacío.</p>";
	} else{
		/* Fin Validacion*/
		if ($imageFileZise>0){
		move_uploaded_file($_FILES["imagefile10"]["tmp_name"], $target_file);
		$imagen=basename($_FILES["imagefile10"]["name"]);
		$img_update="foto10='view/resources/images/documentos/$image_name' ";
...
?>
			<a href="" class="img-responsive" alt="Imagen del factura 1" data-toggle="modal" data-target="#myModal10" style="cursor:pointer;padding: 10px;background: #2ECC71;overflow-y: hidden;overflow-x: hidden;">
...
<?php
			...
			?>
				<img class="img-responsive" src="view/resources/images/documentos/<?php echo $image_name;?>" alt="Imagen del factura origen" data-toggle="modal" data-target="#myModal10" style='cursor:pointer'>
<?php
	}
?>
