	...
	if (empty($_POST['nombre'])) {
		...
		require_once ("../../../config/config.php");//Contiene las variables de configuracion para conectar a la base de datos
           ...
           /* $kind = mysqli_real_escape_string($con,(strip_tags($_POST["kind"],ENT_QUOTES)));*/
			...
			$sql = "INSERT INTO estetica (nombre, apellido, vehiculo, datos, fecha) VALUES('".$nombre."','".$apellido."','".$vehiculo."','".$datos."','".$fecha."');";
			$query_new = mysqli_query($con,$sql);
            // si se ha agregado con éxito
            if ($query_new) {

            		$numeroMaximo="select max(id) as nuevo_estetica from estetica";
            		$idusernew_sql=mysqli_query($con,$numeroMaximo);
            		$idusernew_rw=mysqli_fetch_array($idusernew_sql);
            		$idusernew=$idusernew_rw['nuevo_estetica'];	
